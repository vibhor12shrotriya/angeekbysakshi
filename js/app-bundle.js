...
const Store = (() => {
  const API = 'https://script.google.com/macros/s/AKfycbyh5Sj8LkaVf7hQs6FzwfmqbeFNb5_B8foKKnie-k0-htDVMXkxTvsA15Wo1FnQsXx7/exec'; // ← paste your URL

  // ── Local cache (so UI stays fast) ──
  let cache = {
    customers: null, inventory: null,
    invoices: null,  payments: null,
  };

  // ── Core fetch helpers ──
  async function apiFetch(sheet) {
    const r = await fetch(`${API}?sheet=${sheet}&action=getAll`);
  ...
  async function apiDelete(sheet, id) {
    await fetch(`${API}?sheet=${sheet}&action=delete&id=${id}`);
  }
  ...
  async function init() {
    ...
    try {
      cache.customers = await apiFetch('customers');
      cache.inventory  = await apiFetch('inventory');
      cache.invoices   = await apiFetch('invoices');
      cache.payments   = await apiFetch('payments');
    } catch(e) {
      Utils.toast('Could not connect to database', 'error');
    }
    UI.hideLoading();
  }
  ...
  function getCustomerByMobile(mobile) {
    return getCustomers().find(c => String(c.mobile) === String(mobile).replace(/\D/g,'')) || null;
  }
  async function saveCustomer(cust) {
    await apiSave('customers', cust);
  ...
  function getInventoryByCode(code) {
    return getInventory().find(i => String(i.code).toLowerCase() === String(code).toLowerCase()) || null;
  }
  async function saveInventoryItem(item) {
    await apiSave('inventory', item);
  ...
  async function deleteInventoryItem(id) {
    ...
    cache.inventory = getInventory().filter(i => i.id !== id);
  }
  ...
  async function savePayment(pmt) {
    await apiSave('payments', pmt);
    cache.payments.push(pmt);
  }

  // ── Studio config (keep in localStorage — it's just settings) ──
  function getStudio()     { return JSON.parse(localStorage.getItem('abs_studio') || 'null') || STUDIO; }
  function saveStudio(cfg) { localStorage.setItem('abs_studio', JSON.stringify(cfg)); }

  // ── Invoice numbering (localStorage is fine for this) ──
  function getNextInvNum() { return parseInt(localStorage.getItem('abs_inv_num') || STUDIO.nextInvoiceNum); }
  function bumpInvNum() {
    const n = getNextInvNum();
    localStorage.setItem('abs_inv_num', n + 1);
    return n;
  }
  function formatInvNumber(n) {
    const s = getStudio();
    return `${s.invoicePrefix}-${s.invoiceYear}-${String(n).padStart(3,'0')}`;
  }
  ...
  function exportAll() {
    ...
    a.href = url; a.download = `angeek-backup-${new Date().toISOString().split('T')[0]}.json`;
  ...
  function exportCSV(type) {
    ...
    if (type === 'invoices') {
      headers = ['Invoice#','Type','Customer','Total','Paid','Balance','Status','Date'];
      rows = getInvoices().map(i => {
        const c = Utils.calcInvoice(i);
        return [i.number,i.type,i.customerName,c.total,c.paid,c.balance,i.status,i.createdAt];
      });
    } else if (type === 'customers') {
      headers = ['Name','Mobile','Address','Since'];
      rows = getCustomers().map(c => [c.name,c.mobile,c.address||'',c.since]);
    } else if (type === 'inventory') {
      headers = ['Code','Name','Category','Type','Size','RentPrice','SalePrice','Status'];
    ...
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href = url; a.download = `angeek-${filename}-${new Date().toISOString().split('T')[0]}.csv`;
    a.click(); URL.revokeObjectURL(url);
  }
  ...
  function calcSaleTotal(inv) {
    const price   = Utils.num(inv.salePrice);
    const discAmt = inv.discountType === 'pct' ? Math.round(price * inv.discount / 100) : Utils.num(inv.discount);
  ...
  const UI = {
    showLoading(msg) {
      ...
      if (!el) {
        ...
        el.id = 'db-loading';
        el.style.cssText = 'position:fixed;top:0;left:0;right:0;background:var(--gold);color:#fff;text-align:center;padding:8px;font-size:12.5px;z-index:9999;font-family:var(--font-body)';
